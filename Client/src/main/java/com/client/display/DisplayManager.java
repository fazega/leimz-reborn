...
public class DisplayManager
{
    ...
    private void drawEntities(float scale)
    {
        ...
        //ATTENTION on parcourt ici ligne par ligne et non colonne par colonne pour ̩viter des aberrations d'affichage
                ...
                for(int j = first; j < current_grille.length; j+=2)
                {
                    ...
                    for(int u = 1; u < tile.getTypes().size(); u++)
                    {
                        ...
                        if(type != null)
                        {
                            ...
                            Rectangle ob = new Rectangle(pos_aff.x, pos_aff.y, type.getImg().getWidth(), type.getImg().getHeight()-Base.Tile_y/2);
                    ...
                    for(GroupTiles group : MapManager.instance.getMap_visible().getGroups())
                    {
                        ...
                        if(tile_base.equals(tile))
                        {
                            ...
                            pos_aff.x = (tile_base.getPos_screen().x-(group.getType()).getBase().getX())+((1-scale)*Base.Tile_x);
                            pos_aff.y = (tile_base.getPos_screen().y-(group.getType()).getBase().getY())+((1-scale)*Base.Tile_y/2);

                            //On cree la shape associee pour les collisions
                            Rectangle ob = new Rectangle(pos_aff.x, pos_aff.y, group.getType().getImg().getWidth(), group.getType().getImg().getHeight()-Base.Tile_y/2);
    ...
    public void drawAll(Graphics g, Vector2f mousePos)
    {
        ...
        if(MainJoueur.instance.getEtat().equals(Etat.OVER) || MainJoueur.instance.getEtat().equals(Etat.CLICKED))
        {
            ...
            graphics.drawString(MainJoueur.instance.getPerso().getNom(), MainJoueur.instance.getPos_real_on_screen().x+(MainJoueur.instance.getSize().x/2)-(graphics.getFont().getWidth(MainJoueur.instance.getPerso().getNom())/2), MainJoueur.instance.getPos_real_on_screen().y-20);
        }
        else
        {
            ...
            if(camera.getVisible_entities().get(i) instanceof PNJ)
            {
                ...
                if(pnj.getImgs_repos()!=null)
                {
                    if(pnj.getEtat().equals(Etat.OVER) || pnj.getEtat().equals(Etat.CLICKED))
                    {
                        ...
                        graphics.drawString(pnj.getNom(), pnj.getPos_real_on_screen().x+(pnj.getSize().x/2)-(graphics.getFont().getWidth(pnj.getNom())/2), pnj.getPos_real_on_screen().y-20);
                    }
                    else
                    {
