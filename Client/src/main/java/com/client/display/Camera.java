...
public class Camera
{
    ...
    public void focusOn(Tile tile, Vector2f decalage)
    {
        ...
        indiceFin_x  = (indiceFin_x>=MapManager.instance.getEntire_map().getGrille().length)?MapManager.instance.getEntire_map().getGrille().length:indiceFin_x;
        indiceFin_y  = (indiceFin_y>=MapManager.instance.getEntire_map().getGrille()[0].length)?MapManager.instance.getEntire_map().getGrille()[0].length:indiceFin_y;
        ...
        for(int i = indiceStart_x, y=x_offset; i < indiceFin_x; i++,y++)
        {
            for(int j = indiceStart_y, z=y_offset; j < indiceFin_y; j++, z++)
            {
                ...
                MapManager.instance.getEntire_map().getGrille()[i][j].setPos_screen(
                        new Vector2f(y*Base.Tile_x + decalage_x, z*Base.Tile_y+ shift + decalage_y));
                current_map[y-x_offset][z-y_offset]=MapManager.instance.getEntire_map().getGrille()[i][j];
            }
        }
        ...
        {
            if(group.getBase().getPos().x>=indiceStart_x && group.getBase().getPos().x<=indiceFin_x && group.getBase().getPos().y>=indiceStart_y && group.getBase().getPos().y<=indiceFin_y)
                groups.add(group);
        }
            ...
            for(int j=0; j < current_map[i].length; j++)
            {
                for(int k = 0; k < EntitiesManager.instance.getEntities().size(); k++)
                {
                    if(EntitiesManager.instance.getEntities().get(k)!=null)
                    {
                        ...
                        {
                            if(EntitiesManager.instance.getEntities().get(k).getTile().equals(current_map[i][j]))
                            {
        ...
        MapManager.instance.setAbsolute(current_map[0][0].getPos_screen().copy().sub(current_map[0][0].getPos_real()));
