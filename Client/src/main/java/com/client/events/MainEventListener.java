    ...
    public void pollEvents() {
        if (input.isMousePressed(0)) {
            ...
            for (int i = 0; i < PrincipalGui.instance.getBackside_widgets().size(); i++) {
                if (PrincipalGui.instance.getBackside_widgets().get(i) instanceof ListBox) {
                    GUI_Manager.instance
                            .getRoot()
                            .removeChild(PrincipalGui.instance.getBackside_widgets().get(i));
            ...
            for (int i = 0; i < EntitiesManager.instance.getEntities().size(); i++) {
                Rectangle c =
                        new Rectangle(
                                EntitiesManager.instance.getEntities().get(i).getCorps().getX()
                                        + EntitiesManager.instance
                                                .getEntities()
                                                .get(i)
                                                .getPos_real_on_screen()
                                                .x,
                                EntitiesManager.instance.getEntities().get(i).getCorps().getY()
                                        + EntitiesManager.instance
                                                .getEntities()
                                                .get(i)
                                                .getPos_real_on_screen()
                ...
                if (c.contains(input.getMouseX(), input.getMouseY())) {
                    ...
                    if ((EntitiesManager.instance.getEntities().get(i)) instanceof PNJ) {
                        ...
                        final SimpleChangableListModel<String> l =
                                new SimpleChangableListModel<String>("Parler", "Defier");
                                    ...
                                    public void callback(CallbackReason cbreason) {
                                        if (cbreason == CallbackReason.MOUSE_CLICK) {
                                            ...
                                            if (l.getEntry(id).equals("Parler")) {
                                                final PnjDialogFrame dialog =
                                                        new PnjDialogFrame(pnj);
                                                PrincipalGui.instance.setPnjdialogframe(dialog);
                                                GUI_Manager.instance.getRoot().add(dialog);
                                                dialog.setSize(400, 400);
                                                dialog.setPosition(
                                                        (Base.sizeOfScreen_x / 2)
                                                                - (dialog.getWidth() / 2),
                                                        (Base.sizeOfScreen_y / 2)
                                                                - (dialog.getHeight() / 2));
                                                GUI_Manager.instance
                                                        .getGui()
                                                        .invokeLater(
                        ...
                        if (MainJoueur.instance.getPerso().getCurrent_sort() != null) {
                            String nom_receveur =
                                    ((Joueur) (EntitiesManager.instance.getEntities().get(i)))
                                            .getPerso()
                                            .getNom();
                            String nom_sort =
                                    MainJoueur.instance.getPerso().getCurrent_sort().getNom();
                            NetworkManager.instance.sendToServer(
                                    "a;" + nom_receveur + ";" + nom_sort);
                        } else {
                            if (CombatManager.instance.getCurrent_combat() == null) {
                                CombatManager.instance.askCombat(
                                        (Joueur) EntitiesManager.instance.getEntities().get(i));
                            }
                        }
                    } else {
                        Tile tile_pressed =
                                MapManager.instance.getTileScreen(
                                        new Vector2f(input.getMouseX(), input.getMouseY()));
                        if (tile_pressed != null) {
                            MainJoueur.instance.startMoving(
                                    pf.calculateChemin(
                                            MainJoueur.instance.getTile(), tile_pressed));
                        }
                    }
                }
            }
            ...
            if (!entity_pressed) {
                Tile tile_pressed =
                        MapManager.instance.getTileScreen(
                                new Vector2f(input.getMouseX(), input.getMouseY()));
                if (tile_pressed != null
                        && tile_pressed.getPos().x >= 2
                        && tile_pressed.getPos().x
                                <= MapManager.instance.getEntire_map().getGrille()[0].length - 3
                        && tile_pressed.getPos().y >= 2
                        && tile_pressed.getPos().y
                                <= MapManager.instance.getEntire_map().getGrille()[0].length - 3) {
                    MainJoueur.instance.startMoving(
                            pf.calculateChemin(MainJoueur.instance.getTile(), tile_pressed));
                }
            }
        }
    }
    ...
    public void keyPressed(int key, char c) {
        if (key == Input.KEY_ESCAPE) {
            PrincipalGui.instance
                    .getMenu()
                    .setVisible(!PrincipalGui.instance.getMenu().isVisible());
        ...
        if (key == Input.KEY_I) {
            PrincipalGui.instance
                    .getInventaireUI()
                    .setVisible(!PrincipalGui.instance.getInventaireUI().isVisible());
        ...
        if (key == Input.KEY_P) {
            PrincipalGui.instance
                    .getFencaracs()
                    .setVisible(!PrincipalGui.instance.getFencaracs().isVisible());
            PrincipalGui.instance.getFencaracs().requestKeyboardFocus();
        }

        if (key == Input.KEY_SPACE) {
            if (MainJoueur.instance
                    .getPerso()
                    .getSorts()
                    .get(1)
                    .equals(MainJoueur.instance.getPerso().getCurrent_sort())) {
                MainJoueur.instance.getPerso().setCurrent_sort(null);
            } else {
                MainJoueur.instance
                        .getPerso()
                        .setCurrent_sort(MainJoueur.instance.getPerso().getSorts().get(1));
            }
        }
    }
