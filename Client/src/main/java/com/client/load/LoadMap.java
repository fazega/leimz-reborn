...
public class LoadMap implements Runnable
{
    ...
    public void run()
    {
            ...
            {
                groups.add(new GroupTiles(grille[Integer.parseInt(args_groups[i+1])][Integer.parseInt(args_groups[i+2])], new ArrayList<Tile>(), MapManager.getTypeTile(args_groups[i+3]), Integer.parseInt(args_groups[i])));
            }
            ...
            do
            {
                ...
                for(int u = 1; u < args_tile.length; u+=5)
                {
                    ...
                    for (String layer : args_tile[u+3].split(","))
                        if (!"none".equals(layer)) grille[id_x][id_y].addTypes(MapManager.getTypeTile(layer));
