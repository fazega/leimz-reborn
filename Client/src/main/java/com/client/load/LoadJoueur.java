    ...
    public void run()
    {
            ...
            if(str_i.length>1)
            {
                for(int i = 0; i < str_i.length; i+=4)
                {
                    switch(str_i[i+2])
                    {
                        case "equipement":
                            try {
                                inventaire.addItem(new Equipement(str_i[i], str_i[i+3], str_i[i+1], new Image("data/Images/Objets/"+str_i[i].toLowerCase().replace(" ", "_")+".png"), new Image("data/Images/Objets/"+str_i[i].toLowerCase().replace(" ", "_")+".png"), null, 20));
                        ...
                        case "objet_quete":
                            try {
                                inventaire.addItem(new ObjetQuete(str_i[i], str_i[i+1], new Image("data/Images/Objets/"+str_i[i].toLowerCase().replace(" ", "_")+".png"), new Image("data/Images/Objets/"+str_i[i].toLowerCase().replace(" ", "_")+".png"), null, 20));
                            } catch (SlickException e) {
                                e.printStackTrace();
                            }
                            break;
            ...
            for(int i = 1; i < qm.length; i++)
            {
                ...
                for(int j = 2; j < str_q.length; j+=5)
                {
                    ...
                    if(str_q[j+4].equals("1"))
                        quete_o.setAccompli(true);
                    ql.add(quete_o);
                }
