...
public class MapManager implements NetworkListener {
    ...
    public static void initTypesTile() {
        ...
        for (int i = 0; i < racine.getChild("calque1").getChildren().size(); i++) {
            info_tile[5] = "1";
            info_tile[0] =
                    ((Element) racine.getChild("calque1").getChildren().get(i))
                            .getChild("nom")
                            .getText();
            info_tile[1] =
                    ((Element) racine.getChild("calque1").getChildren().get(i))
            ...
            try {
                t =
                        new TypeTile(
                                info_tile[0],
                                new Image(info_tile[1]),
                                new Rectangle(
                                        Integer.parseInt(info_tile[3]),
                                        Integer.parseInt(info_tile[4]),
                                        80,
                                        40),
                                Boolean.parseBoolean(info_tile[2]),
                                Integer.parseInt(info_tile[5]));
        ...
        for (int j = 0; j < map.getGrille()[0].length; j++) {
            ...
            for (int k = 0; k < 2; k++) {
                for (int i = first; i < map.getGrille().length; i += 2) {
                    ...
                    // La position vaut la position de la tile moins la position de la base, creee
                    // auparavant par le level designer pour chaque objet
                    // De plus, on ajoute une petite formule demontrable simplement en repere
                    // orthonorme pour le zoom
                    pos_aff.x =
                            (tile.getPos_screen().x
                                            - ((TypeTile) tile.getTypes().get(0)).getBase().getX())
                                    + ((1 - scale) * Base.Tile_x);
                    pos_aff.y =
                            (tile.getPos_screen().y
                                            - ((TypeTile) tile.getTypes().get(0)).getBase().getY())
                                    + ((1 - scale) * Base.Tile_y / 2);
                    if (filter != null) {
                        ((TypeTile) tile.getTypes().get(0))
                                .getImg()
                                .draw(pos_aff.x, pos_aff.y, scale, filter);
                    } else {
                        ((TypeTile) tile.getTypes().get(0))
                                .getImg()
                                .draw(pos_aff.x, pos_aff.y, scale);
                    }
            ...
            for (int j = 0; j < entire_map.getGrille()[i].length; j++) {
                ...
                if (i % 2 == 0) // Si la ligne est paire
                {
                    ...
                    entire_map.getGrille()[i][j].setPos_screen(
                            new Vector2f(
                                    (entire_map.getGrille()[i][j].getPos().x / 2) * 80,
                                    entire_map.getGrille()[i][j].getPos().y * 40));
                ...
                } else // Si la ligne est impaire
                {
                    ...
                    entire_map.getGrille()[i][j].setPos_screen(
                            new Vector2f(
                                    (entire_map.getGrille()[i][j].getPos().x * 80) / 2,
                                    (entire_map.getGrille()[i][j].getPos().y * 40) + 20));
                    entire_map.getGrille()[i][j].setPos_real(
                            new Vector2f(
                                    (entire_map.getGrille()[i][j].getPos().x * 80) / 2,
                                    (entire_map.getGrille()[i][j].getPos().y * 40) + 20));
    ...
    public ArrayList<ArrayList<Tile>> getTilesAutour(Tile tile, int spread) {
        ...
        for (int i = indiceStart_x; i < indiceFin_x; i++) {
            ...
            for (int j = indiceStart_y; j < indiceFin_y; j++) {
                ...
                if (i >= 0
                        && j >= 0
                        && i < this.getEntire_map().getGrille().length
                        && j < this.getEntire_map().getGrille()[i].length) {
