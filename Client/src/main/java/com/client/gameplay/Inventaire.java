    ...
    public void receiveMessage(String str) {
        ...
        if(temp[0].equals("new"))
        {
            switch(temp[3])
            {
                case "equipement":
                    try {
                        items.add(new Equipement(temp[1], temp[4], temp[2], new Image("data/Images/Objets/anneau_dfer.png"),new Image("data/Images/Objets/anneau_dfer.png"), null, 0));
                ...
                case "objet_quete":
                    try {
                        items.add(new ObjetQuete(temp[1], temp[2], new Image("data/Images/Objets/anneau_dfer.png"),new Image("data/Images/Objets/anneau_dfer.png"), null, 0));
                    } catch (SlickException e) {
                        e.printStackTrace();
                    }
                    break;
            }
            PrincipalGui.instance.getChat_frame().appendRow("font_green", "[Objet re̤u : "+temp[1]+"]");
        }
        if(temp[0].equals("del"))
        {
            ...
            items.remove(item);
            PrincipalGui.instance.getChat_frame().appendRow("font_green", "[Objet donn̩ : "+temp[1]+"]");
