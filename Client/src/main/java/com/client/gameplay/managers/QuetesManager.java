...
public class QuetesManager implements NetworkListener
{
    ...
    public void receiveMessage(String str) {
        ...
        if(temp[0].equals("new"))
        {
            ...
            frame.adjustSize();
            frame.setPosition(Base.sizeOfScreen_x/2-frame.getWidth()/2, Base.sizeOfScreen_y/2-frame.getHeight()/2);
    ...
    public static ResizableFrame newQueteFrame(Quete quete)
    {
        ...
        l.setHorizontalGroup(l.createParallelGroup().addWidget(labN).addWidget(labC).addWidget(lb).addWidget(okButton, Alignment.CENTER));
        l.setVerticalGroup(l.createSequentialGroup().addWidget(labN).addWidget(labC).addGap(40).addWidget(lb).addGap().addWidget(okButton));
