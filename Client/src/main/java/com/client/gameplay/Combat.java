    ...
    public void draw(Graphics g, Vector2f mousePos, float scale) {
        ...
        if (this.getEtat().equals(EtatCombat.EN_COURS)) {
            for (int i = 0; i < this.getZone().size(); i++) {
                ...
                // De plus, on ajoute une petite formule demontrable simplement en repere orthonorme
                // pour le zoom
                pos_aff.x =
                        (this.getZone().get(i).getPos_screen().x
                                        - ((TypeTile) this.getZone().get(i).getTypes().get(0))
                                                .getBase()
                                                .getX())
                                + ((1 - scale) * Base.Tile_x);
                pos_aff.y =
                        (this.getZone().get(i).getPos_screen().y
                                        - ((TypeTile) this.getZone().get(i).getTypes().get(0))
                                                .getBase()
                                                .getY())
            ...
            if (this.getTimer_start().getTime() > 0) {
                ...
                g.drawString(
                        txt,
                        Base.sizeOfScreen_x / 2 - (label.getWidth(txt) / 2),
                        Base.sizeOfScreen_y / 2 - (label.getHeight(txt) / 2));
            }
