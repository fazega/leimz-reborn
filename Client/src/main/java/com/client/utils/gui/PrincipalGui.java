...
public class PrincipalGui implements NetworkListener {
    ...
    public static ResizableFrame getPopup(String text) {
        ...
        panel.setVerticalGroup(
                panel.createSequentialGroup().addWidget(label).addGap(25).addWidget(button));

        frame.add(panel);

        frame.setPosition(
                (Base.sizeOfScreen_x / 2) - (frame.getPreferredWidth() / 2),
                (Base.sizeOfScreen_y / 2) - (frame.getPreferredHeight() / 2) + 50);
        panel.adjustSize();
    ...
    private void init() {
        ...
        inventaireUI.setPosition(
                (Base.sizeOfScreen_x - (inventaireUI.getWidth())) / 2,
                (Base.sizeOfScreen_y - (inventaireUI.getHeight())) / 2);
        ...
        fencaracs.setPosition(
                (Base.sizeOfScreen_x - (fencaracs.getWidth())) / 2,
                (Base.sizeOfScreen_y - (fencaracs.getHeight())) / 2);
        ...
        menu.adjustSize();
        menu.setPosition(
                (Base.sizeOfScreen_x / 2) - (menu.getWidth() / 2),
                (Base.sizeOfScreen_y / 2) - (menu.getHeight() / 2));
        menu.setVisible(false);
    }
        ...
        for (int i = 0; i < inventory_panel.getSlots().size(); i++) {
            for (int j = 0; j < inventory_panel.getSlots().get(i).size(); j++) {
                if (inventory_panel.getSlots().get(i).get(j).isFenOk()) {
                    ResizableFrame panInfo =
                            InventaireUI.panInfo(
                                    inventory_panel.getSlots().get(i).get(j).getItem());

                    GUI_Manager.instance.getRoot().add(panInfo);

                    panInfo.setPosition(
                            (inventory_panel.getX() + (inventory_panel.getWidth() / 2))
                                    - (panInfo.getWidth() / 2),
                            (inventory_panel.getY() + (inventory_panel.getHeight() / 2))
                                    - (panInfo.getHeight() / 2));
        ...
        if (type == YES_NO_PANE) {
            ...
            l.setHorizontalGroup(
                    l.createParallelGroup()
                            .addWidget(label)
                            .addGroup(l.createSequentialGroup(ouiB, nonB)));
            l.setVerticalGroup(
                    l.createSequentialGroup()
                            .addWidget(label)
                            .addGroup(l.createParallelGroup(ouiB, nonB)));
