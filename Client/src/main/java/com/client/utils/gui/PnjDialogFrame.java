...
public class PnjDialogFrame extends ResizableFrame implements NetworkListener
{
    ...
    private void init()
    {
        ...
        lb.addCallback(new CallbackWithReason<ListBox.CallbackReason>() {
            ...
            public void callback(CallbackReason cbreason)
            {
                ...
                {
                    NetworkManager.instance.sendToServer("pd;"+pnj.getNom()+";"+encours.getReponses().get(lb.getSelected()).getId());
                    return;
                }
            }
    ...
    public void receiveMessage(String str) {
        ...
        {
            this.pnj.setPnjDiscours(new PNJ_discours(temp[1], null, new ArrayList<PNJ_discours>(), Integer.parseInt(temp[0])));
            for(int i = 2; i < temp.length; i+=3)
            {
                PNJ_discours r = new PNJ_discours(temp[i+2], this.pnj.getPnjDiscours(), new ArrayList<PNJ_discours>(), Integer.parseInt(temp[i]));
        ...
        else
        {
            ...
            parent.addReponses(new PNJ_discours(temp[2], parent, new ArrayList<PNJ_discours>(), Integer.parseInt(temp[1])));
            for(int i = 3; i < temp.length; i+=3)
            {
                PNJ_discours r = new PNJ_discours(temp[i+2], parent.getReponses().get(0), new ArrayList<PNJ_discours>(), Integer.parseInt(temp[i]));
