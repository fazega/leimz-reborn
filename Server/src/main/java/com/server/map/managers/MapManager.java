...
public class MapManager
{
        ...
        for(int i = 0; i < entire_map.getGrille().length; i++)
        {
            for(int j = 0; j <entire_map.getGrille()[i].length; j++)
            {
                if(i%2 ==0) //Si la colonne est paire
                {
                    ...
                    entire_map.getGrille()[i][j].setPos_real_x((((float)entire_map.getGrille()[i][j].getPos_x())/2)*80);
                    entire_map.getGrille()[i][j].setPos_real_y((((float)entire_map.getGrille()[i][j].getPos_y()))*40);
                }
                ...
                else //Si la ligne est impaire
                {
                    //On dï¿½termine la "vraie" position de la tile par rapport aux coordonnï¿½es
                    entire_map.getGrille()[i][j].setPos_real_x((((float)entire_map.getGrille()[i][j].getPos_x())/2)*80);
                    entire_map.getGrille()[i][j].setPos_real_y((((float)entire_map.getGrille()[i][j].getPos_y())*40)+20);
                }
    ...
    public Tile getTileScreen(float pos_screen_x, float pos_screen_y)
    {
        ...
        { //Parcours toute la grille visible
            ...
            {
                if(this.entire_map.getGrille()[i][j].isPointed(pos_screen_x,pos_screen_y) && this.entire_map.getGrille()[i][j].isDrawn())
    ...
    public Tile getTileReal(float pos_real_x, float pos_real_y)
    {
        ...
        { //Parcours toute la grille visible
            for(int j=0; j < this.entire_map.getGrille()[i].length; j++)
            {
                ...
                if(tile.isPointed(pos_real_x,pos_real_y,tile.getPos_real_x(), tile.getPos_real_y()))
                    return tile;
            }
        }
        return null;
    ...
    public Tile[][] getTilesAutour(int pos_x, int pos_y, int spread)
    {
        ...
        int indiceFin_x = ((int) (indiceStart_x + spread*2))<= this.getEntire_map().getGrille().length-1?(indiceStart_x + spread*2):this.getEntire_map().getGrille().length-1;
        int indiceFin_y = ((int) (indiceStart_y + spread*2))<=this.getEntire_map().getGrille().length-1?(indiceStart_y + spread*2):this.getEntire_map().getGrille().length-1;
        ...
        for(int i = 0; i < indiceFin_x-indiceStart_x; i++)
        {
            if(i >= 0 && i < this.getEntire_map().getGrille().length)
            {
                for(int j = 0; j < indiceFin_y-indiceStart_y; j++)
                {
                    ...
                    {
                        grille[i][j]=this.getEntire_map().getGrille()[i+indiceStart_x]
                                [j+indiceStart_y];
                    }
                }
            }
