...
public class ClientsManager {
    ...
    public ClientList findProperList() {
        ...
        for (ClientList c : lists) {
            // Si le nombre de client present dans une liste est inferieur au nombre maximum de
            // client par thread.On se prepare a  retourner la liste
            if (c.getClients().size() < GlobalConstant.maxClientsPerThread) toreturn = c;
        }
    ...
    public ArrayList<Client> getClientsNear(Client client) {
        ArrayList<Joueur> entities_near =
                EntitiesManager.instance.getPlayersAround(client.getCompte().getCurrent_joueur());
