...
public class PnjDialogFunction implements Functionable
{
    ...
    public void doSomething(String[] args, Client c)
    {
        ...
        {
            if(EntitiesManager.instance.getPnjs_manager().getPnjs().get(i).getNom().equals(args[1]))
            {
                pnj = EntitiesManager.instance.getPnjs_manager().getPnjs().get(i);
            }
        }
        //Si on ne l'a pas trouvé ou qu'il n'a pas encore été loadé, on demande au joueur de ne pas jouer au hacker ...
        if(pnj==null || !c.getCompte().getCurrent_joueur().getLoaded_entities().contains(pnj))
        {
            c.sendMessage("err;Vous essayez d'accéder à un pnj que vous n'avez même pas en vue ... Attention, le hackage est interdit, et puni de bannissement.");
            c.disconnect();
            return;
        }
        ...
        ArrayList<Quete> qlist = c.getCompte().getCurrent_joueur().getPerso().getQuetes_manager().getQuetes();
        //La liste suivante va contenir les ids de tous les objectifs de ces quêtes, ce qui va permettre
        ...
        {
            //On parcourt les dicours pour éventuellement tomber sur un qui correspond à un objectif de quête
            ...
            if(discours == null)
                discours = pnj.getDiscours().get(0);
        }
        ...
        else
        {
            ...
            if(pnj.getQuetes().containsKey(Integer.parseInt(args[2])))
            {
                ...
                if(!in)
                {
                    c.getCompte().getCurrent_joueur().getPerso().getQuetes_manager().addQuete(quete);
                    //On crée le message de quête (assez explicite, obj veut dire objectif)
                    String message ="que;new;"+pnj.getNom()+";"
                            +quete.getNom()+";"+"obj;";
                    try {
                        String sql = "INSERT INTO quetes_joueur VALUES(\""+c.getCompte().getCurrent_joueur().getPerso().getNom()+"\", "+quete.getId()+", \""+pnj.getNom()+"\", 0)";
        ...
        for(Quete quete : qlist)
        {
            ...
            if(!quete.isFinie())
            {
                                    ...
                                    case "parler_a":
                                        if(obj.getObjectifs().get(0).equals(pnj))
                                        {
                                            try {
                                                ...
                                                String sql = "INSERT INTO quetes_joueur_objectifs_accomplis VALUES(\""+c.getCompte().getCurrent_joueur().getPerso().getNom()+"\", "+
                                                        quete.getId()+","+ obj.getId()+", NULL)";
                                                Statement stmt = ServerSingleton.getInstance().getDbConnexion().getConnexion().createStatement();
                                                stmt.executeUpdate(sql);
                                            } catch (SQLException e) {
                                                e.printStackTrace();
                                            ...
                                            c.getCompte().getCurrent_joueur().getPerso().getInventaire().getItems().addAll(obj.getRecompenses());
                                            for(SimpleItem item : obj.getRecompenses())
                                            {
                                                System.out.println("New item");
                                                //Ce code vérifiait que l'objet était ou non dans l'inventaire, ce qui me paraîssait bizarre ... Je l'ai commenté.
                                                ...
                                                {
                                                    try {
                                                        String sql = "INSERT INTO inventaire VALUES(\""+c.getCompte().getCurrent_joueur().getPerso().getNom()+"\", \""+
                                                                item.getNom()+"\", 1)";
                                                        Statement stmt = ServerSingleton.getInstance().getDbConnexion().getConnexion().createStatement();
                                                        stmt.executeUpdate(sql);
                                                    } catch (SQLException e) {
                                                    ...
                                                    if(item instanceof ObjetQuete)
                                                        c.sendMessage("in;new;"+item.getNom()+";"+item.getDescription()+";objet_quete;;");
                                                    if(item instanceof Equipement)
                                                        c.sendMessage("in;new;"+item.getNom()+";"+item.getDescription()+";equipement;"+((Equipement)item).getType());												}

                                            }
                                            c.sendMessage("que;update;"+quete.getId()+"acc;obj;"+obj.getId());
                                        }
                                        break;
                                    case "don_objet_a":
                                        if(obj.getObjectifs().get(0).equals(pnj) && c.getCompte().getCurrent_joueur().getPerso().getInventaire().getItems().contains(obj.getObjectifs().get(1)))
                                        {
                                            try {
                                                String sql = "INSERT INTO quetes_joueur_objectifs_accomplis VALUES(\""+c.getCompte().getCurrent_joueur().getPerso().getNom()+"\", "+
                                                        quete.getId()+","+ obj.getId()+", NULL)";
                                            ...
                                            try {
                                                String sql = "DELETE FROM inventaire WHERE objet=\""+((SimpleItem)obj.getObjectifs().get(1)).getNom()+"\"";
                                                Statement stmt = ServerSingleton.getInstance().getDbConnexion().getConnexion().createStatement();
                                                stmt.executeUpdate(sql);
                                            } catch (SQLException e) {
                                                e.printStackTrace();
                                            }
                                            c.sendMessage("in;del;"+((SimpleItem)obj.getObjectifs().get(1)).getNom());
                                            obj.setAccompli(true);
                                            c.getCompte().getCurrent_joueur().getPerso().getInventaire().getItems().remove(obj.getObjectifs().get(1));
                                            c.getCompte().getCurrent_joueur().getPerso().getInventaire().getItems().addAll(obj.getRecompenses());
                                            c.sendMessage("que;update;"+quete.getId()+"acc;obj;"+obj.getId());
                                        }
                                        else
                ...
                if(finie)
                {
                    try {
                        String sql = "INSERT INTO quetes_joueur_objectifs_accomplis VALUES(\""+c.getCompte().getCurrent_joueur().getPerso().getNom()+"\", "+
                                quete.getId()+","+ quete.getObjectifs().get(quete.getObjectifs().size()-1).getId()+", NULL)";
                        Statement stmt = ServerSingleton.getInstance().getDbConnexion().getConnexion().createStatement();
                        stmt.executeUpdate(sql);
                    } catch (SQLException e) {
                        e.printStackTrace();
                    ...
                    try {
                        String sql = "UPDATE quetes_joueur SET finie=1 WHERE id_quete="+quete.getId();
                        Statement stmt = ServerSingleton.getInstance().getDbConnexion().getConnexion().createStatement();
                        stmt.executeUpdate(sql);
                    } catch (SQLException e) {
                        e.printStackTrace();
