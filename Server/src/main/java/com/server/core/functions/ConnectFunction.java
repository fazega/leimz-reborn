    ...
    public void doSomething(String[] args, Client c) {
        if (args.length < 2)
            ServerSingleton.getInstance()
                    .printMessage("Erreur a la connection : message trop court.");
        ...
        try {
            ...
            rsj =
                    stmt.executeQuery(
                            "SELECT currjoueur,connected FROM account "
                                    + "WHERE nom_de_compte='"
                                    + ndc
                                    + "' "
                                    + "AND mot_de_passe='"
        ...
        try {
            rsj.close();
            stmt.executeUpdate(
                    "UPDATE account SET connected=true WHERE nom_de_compte='" + ndc + "'");
        } catch (SQLException e) {
            ServerSingleton.getInstance()
                    .printMessage("Finding player statement's creation failed");
            return;
        }
        ...
        ServerSingleton.getInstance()
                .printMessage(
                        "Joueur "
                                + c.getCompte().getName()
                                + " connecte. IP = "
                                + c.getS().getLocalAddress().toString()
                                + ". Actuellement il y a "
                                + n
                                + " joueur"
                                + (n > 1 ? "s" : "")
