...
public class CombatFunction implements Functionable
{
    ...
    private void answerAskingFunction(Client c, String[] args)
    {
        ...
        if(args[2].equals("y"))
        {
            receiver.sendMessage("s;j;"+c.getCompte().getCurrent_joueur().getPerso().getNom()+";vie;"+c.getCompte().getCurrent_joueur().getPerso().getCaracs_values().get(Caracteristique.VIE));
            c.sendMessage("s;j;"+receiver.getCompte().getCurrent_joueur().getPerso().getNom()+";vie;"+receiver.getCompte().getCurrent_joueur().getPerso().getCaracs_values().get(Caracteristique.VIE));
            receiver.sendMessage("fi;an;y;"+c.getCompte().getCurrent_joueur().getPerso().getNom());
        }
        else if(args[2].equals("n"))
        {
            receiver.sendMessage("fi;an;n;"+c.getCompte().getCurrent_joueur().getPerso().getNom());
        }
