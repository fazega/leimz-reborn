...
public class StateFunction implements Functionable
{
    ...
    public void doSomething(String[] args, Client client)
    {
        ...
        {
            try {
                Tile tile = MapManager.instance.getTileReal(Float.parseFloat(args[2]), Float.parseFloat(args[3]));
                ...
                stmt.executeUpdate("UPDATE personnage " +
                        " SET posx="+tile.getPos_x()+" , posy="+tile.getPos_y()+
                ...
                if(tile != client.getCompte().getCurrent_joueur().getTile())
                    ServerSingleton.getInstance().printMessage("Position du joueur "+client.getCompte().getCurrent_joueur().getPerso().getNom()+
                            ": "+tile.getPos_x()+" -- "+tile.getPos_y());
