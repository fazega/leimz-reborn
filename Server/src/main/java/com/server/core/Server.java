...
public class Server
{
    ...
    private void initServer()
    {
        ...
        {
            // On lance le socket
            ss = new ServerSocket(Integer.getInteger("leimz.port", 1500), 50, java.net.InetAddress.getByName("127.0.0.1"));
    ...
    private void waitPlayers()
    {
        ...
        while (true) // attente en boucle de connexion (bloquant sur ss.accept)
        {
            try
            {
                ...
                //On l'ajoute
                clientl.addClient(new Client(player,ClientsManager.instance.getLists().indexOf(clientl)));
    ...
    public void deconnexion(Client client)
    {
        ...
        ServerSingleton.getInstance().printMessage("Joueur "+client.getCompte().getName()+" deconnecte. IP = "+client.getS().getLocalAddress().toString()+". Actuellement il y a "+(n-1)+" joueur"+(n>1?"s":"")+".");

        if(client.getCompte().getCurrent_joueur() != null)
        {
            for(Client c : ClientsManager.instance.getClients())
            {
                if(c.getCompte().getCurrent_joueur() != null && c.getCompte().getCurrent_joueur().getLoaded_entities().contains(client.getCompte().getCurrent_joueur()))
                {
                    c.getCompte().getCurrent_joueur().getLoaded_entities().remove(client.getCompte().getCurrent_joueur());
                    c.sendMessage("s;j;"+client.getCompte().getCurrent_joueur().getPerso().getNom()+";deco;");
                }
            }
