    ...
    public static boolean intersect(Polygon polygon1, Polygon polygon2)
    {
        for(int i = 0; i < polygon1.getPoints().size()-1; i++)
        {
            ...
            for(int j = 0; j < polygon2.getPoints().size()-1; j++)
            {
                ...
                if(intersectDroite(droite1, droite2))
                    return true;
            }
        }
        ...
        if(point1a.x!=point1b.x)
        {
            ...
            if(point2a.x != point2b.x)
            {
                ...
                if((m2-m1)!=0)
                {
                    ...
                    {
                        if(inter.x >= point2b.x && inter.x <= point2a.x)
                            return true;
                    }
                    else
                    {
                        if(inter.x >= point2a.x && inter.x <= point2b.x)
                            return true;
                    }
                }
                else
            ...
            else
            {
                ...
                {
                    if(inter.x >= point1b.x && inter.x <= point1a.x)
                        return true;
                }
                else
                {
                    if(inter.x >= point1a.x && inter.x <= point1b.x)
                        return true;
                }
            }
        }
